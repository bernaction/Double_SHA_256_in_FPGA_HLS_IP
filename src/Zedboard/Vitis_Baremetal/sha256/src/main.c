...
#define SHA256_BASE       	0x40000000  // s_axi_control
#define GPIO_BASE           0x40010000// Endereços para o AXI GPIO
...
void IP_Mining(const char *header_hex, const char *hash_real) {
  ...
  nanos_fpga = ((u64)cycles * 1000000000ULL) / 100000000ULL; // clk = 100 MHz
  hashrate_fpga = (nanos_fpga > 0) ? 1000000000ULL / nanos_fpga : 0;
}

void resultados() {
  xil_printf("\r\nIniciando Double SHA256 no CPU (Cortex-A9 arch:ARMv71)...");
  xil_printf("\r\nDouble Hash gerado (em LE): %s", hash_string);

  xil_printf("\r\n\nIniciando Double SHA256 no FPGA (ZedBoard Zynq-7000)...");
  xil_printf("\r\nDouble Hash gerado (em LE): %s", hash_string);
