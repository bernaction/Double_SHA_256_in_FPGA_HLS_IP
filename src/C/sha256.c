...
/*Um vetor de entrada (data[]) e seu tamanho (len)
...
void sha256_update(uchar in_data[64], uint *datalen, uint bitlen[2], const uchar data[], uint len, uint state[8]) {
  for (uint i = 0; i < len; ++i) {
    in_data[*datalen] = data[i];
    (*datalen)++;
    if (*datalen == 64) {
