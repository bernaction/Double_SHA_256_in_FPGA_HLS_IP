...
\subsection{Dificuldade da Rede}\label{subsec:diff_rede}
...
\newpage
Exemplo de código para o cálculo da dificuldade com ajuste de bits
\begin{lstlisting}[language=C++, label={lst:difficulty_code}]
#include <iostream>
#include <cmath>
...
\end{equation}
\addEquacao{Dificuldade atual}{1}
...
\end{equation}
\addEquacao{Probabilidade da bifurcação}{2}
...
\begin{equation}
\lambda_{\text{mine}} = \frac{\text{velocidade computacional}}{\text{dificuldade de mineração}}.
\end{equation}
\addEquacao{Probabilidade de gerar um bloco}{3}
...
\begin{equation}
\text{mining-diff} \geq -\frac{\text{(velocidade computacional)} \cdot \sum_{j=1}^{\lceil d_{\text{evol-rand}} \rceil} shd \cdot \psi^j}{\ln(1-\text{alvo})}.
\end{equation}
\addEquacao{Dificuldade mínima aceitável}{4}

\subsection{Eficiência Energética na Mineração}\label{subsec:eficiencia_energetica}

A eficiência energética é um parâmetro fundamental na avaliação de equipamentos de mineração, por indicar quão eficazmente um dispositivo converte energia elétrica em poder de \textit{hashing}. A métrica Watt/TH/s (Watts por Terahash por segundo) é amplamente utilizada para comparar dispositivos distintos, mostrando quantos watts são necessários para gerar um terahash de poder de mineração em um segundo.

A métrica de eficiência energética na mineração de Bitcoin é frequentemente expressa em Watts por Terahash por segundo (Watt/TH/s). Inicialmente, quando a mineração ainda era realizada por CPUs e, logo depois, por GPUs, as unidades de medida menores, como MH/s (megahashes por segundo) ou GH/s (gigahashes por segundo), eram comuns. No entanto, à medida que a dificuldade da rede, explicada na \autoref{subsec:diff_rede}, aumentou e a capacidade computacional dos equipamentos se tornou exponencialmente maior, o padrão passou a ser o TH/s. Isso se deve à escala massiva de poder de processamento agora disponível com equipamentos baseados em ASICs e FPGAs otimizados, tornando o TH/s uma unidade mais adequada para quantificar o desempenho da mineração em larga escala.

Uma eficiência energética menor (em Watt/TH/s) indica que o equipamento precisa de menos energia para produzir um determinado \textit{hashrate}, tornando-o mais competitivo e reduzindo os custos operacionais a longo prazo. Por exemplo, equipamentos baseados em ASICs mais modernos apresentam valores de Watt/TH/s bastante baixos, evidenciando altos níveis de otimização de seu hardware dedicado ao \textit{hashing} do algoritmo Double SHA-256.

O \autoref{tab:eficiencia_mineracao} apresenta um comparativo de diferentes equipamentos de mineração, destacando seu \textit{hashrate}, consumo energético e eficiência energética. Observa-se que soluções especializadas, como ASICs de última geração, apresentam níveis de eficiência muito superiores a CPUs e GPUs.

\begin{table}[H]
    \caption{Comparativo de Eficiência Energética entre Diferentes Equipamentos de Mineração}
    \label{tab:eficiencia_mineracao}
    \centering
    ...
    \textbf{Equipamento} & \textbf{Hashrate (TH/s)} & \textbf{Energia (W)} & \textbf{Eficiência (Watt/TH/s)} \\ \hline
    CPU: Ryzen 9 5950x & 0,0003 & 175 & 583.333,33 \\ \hline
    GPU: NVIDIA RTX 3080 Ti & 0,003825 & 334 & 87.320,26 \\ \hline
    GekkoScience CompacF (BM1397 — Antminer S17) & 0,350 & 20 & 57,14 \\ \hline
    Bitaxe Ultra ou NerdAxe (BM1366 — Antminer S19) & 0,550 & 25 & 45 \\ \hline
    Bitaxe Supra (BM1368 — Antminer S21) & 0,782 & 16,4 & 20,20 \\ \hline
    Bitaxe Gamma (BM1370 — Antminer S21 Pro) & 1,200 & 18 & 15 \\ \hline
    ASIC: Bitmain AntMiner S21 Pro & 234 & 3531 & 15 \\ \hline
    \end{tabular}
...
\subsection*{Double SHA-256}
O Bitcoin utiliza o algoritmo de hash SHA-256 duas vezes consecutivas sobre o mesmo bloco de dados, prática conhecida como \textbf{Double SHA-256}. O processo é definido como:

\begin{center}
    \texttt{hash = SHA256(SHA256(header))}
\end{center}

Essa escolha, adotada por Satoshi Nakamoto na implementação original do Bitcoin \cite{nakamoto2008bitcoin}, não é explicitamente justificada no whitepaper. No entanto, discussões técnicas posteriores indicam que o uso do Double SHA-256 visa aumentar a segurança do sistema contra ataques de extensão de comprimento (\textit{length-extension attacks}), uma vulnerabilidade inerente ao esquema Merkle–Damgård utilizado pelo SHA-256 \cite{BitcoinWiki}.

Uma vulnerabilidade conhecida em funções hash baseadas na construção Merkle–Damgård, como o SHA-256, é o \textit{length-extension attack} (ataque de extensão de comprimento). Nesse tipo de ataque, um adversário que conhece apenas o valor de \texttt{hash(mensagem)} e o comprimento da mensagem original pode forjar o valor de \texttt{hash(mensagem || dados\_adicionais)}, sem a necessidade de conhecer a mensagem original. Isso ocorre porque o estado interno do hash, ao final do processamento da mensagem legítima, pode ser reutilizado para continuar o cálculo sobre dados adicionais com o mesmo efeito criptográfico. Para mitigar esse risco, o algoritmo de prova-de-trabalho do Bitcoin aplica a função SHA-256 duas vezes consecutivas, protegendo contra ataques de extensão de comprimento e impedindo que terceiros consigam gerar hashes válidos para mensagens estendidas sem possuir os dados originais. Além disso, a aplicação dupla da função dificulta ataques pré-computados (\textit{preimage attacks}) e aumenta a imprevisibilidade do resultado final, uma vez que o segundo SHA-256 opera sobre uma entrada de tamanho fixo e padronizado, eliminando efeitos colaterais relacionados ao preenchimento interno. Essa vulnerabilidade e sua mitigação no contexto do Bitcoin são discutidas detalhadamente em \cite{CryptologieLengthExtension}.


\begin{figure}[H]
    \centering
    \caption{Implementação do Double SHA-256 em uma mensagem.}
    \includegraphics[width=0.85\linewidth]{2_fundamentacao/double_sha256_fluxo.pdf}
    \caption*{Fonte: O autor (2025).}
    \label{fig:double-sha-256}
\end{figure}
Analisando o fluxograma, após a primeira rodada de hash (\texttt{SHA256(header)}) é necessário realizar uma limpeza dos buffers utilizados no processo antes de executar a segunda rodada. Essa etapa garante que nenhuma informação residual da primeira iteração interfira no segundo cálculo. As variáveis e estruturas envolvidas são descritas a seguir:

\begin{itemize}
  \item \textbf{\texttt{in\_data[]}}: vetor de 64 bytes usado como bloco temporário de entrada para o algoritmo SHA-256. Deve ser zerado para remover qualquer resíduo do \texttt{header} original que possa afetar o segundo hash.

  \item \textbf{\texttt{datalen}}: variável que armazena a quantidade de bytes válidos atualmente armazenados em \texttt{in\_data}. É resetada para zero antes do novo preenchimento com o \texttt{hash1}.

  \item \textbf{\texttt{bitlen[2]}}: vetor de 64 bits (dividido em duas posições de 32 bits) que representa o tamanho total dos dados em bits. Também deve ser zerado, pois o segundo \texttt{sha256\_update()} utilizará um novo comprimento de entrada (32 bytes = 256 bits).

  \item \textbf{\texttt{state[8]}}: vetor de oito palavras de 32 bits usado internamente para manter o estado do algoritmo durante o processamento. Ele é reinicializado com os valores padrão do SHA-256 através da chamada \texttt{sha256\_init()}, tanto antes da primeira quanto antes da segunda rodada de hash.

\end{itemize}

Essa limpeza explícita dos dados garante a independência entre as duas rodadas do algoritmo. Na primeira rodada, o vetor \texttt{in\_data} recebe os 80 bytes do cabeçalho do bloco (\texttt{header}), e na segunda, recebe os 32 bytes de saída da primeira rodada (\texttt{hash1}). O resultado final, \texttt{hash2}, é o valor de \texttt{SHA256(SHA256(header))}, utilizado como prova de trabalho no protocolo Bitcoin.

\subsection*{Código base}

Como exemplo, o pseudocódigo apresentado na Listagem \ref{lst:sha256_transform} foi utilizado como base para a configuração do algoritmo SHA-256 no FPGA. Esse algoritmo, descrito em linguagem C, foi adaptado para o desenvolvimento em HLS, possibilitando a conversão para descrição de hardware em nível de registro RTL. A utilização dessa abordagem permitiu explorar o desempenho do FPGA.
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.40\linewidth]{2_fundamentacao/fpga_layout.png}
